...
# Google Wallet Payment Endpoints
@api_router.post("/payments/google-pay/create-session")
async def create_google_pay_session(request: dict):
    ...
    try:
        ...
        session_data = await google_wallet_service.create_google_pay_session(
            amount, currency, trainer_id, client_email, session_details
        )
        
        if session_data:
            return session_data
        else:
            raise HTTPException(status_code=500, detail="Failed to create Google Pay session")
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@api_router.post("/payments/google-pay/process")
async def process_google_pay_payment(request: dict):
    """Process Google Pay payment"""
    try:
        payment_token = request.get("payment_token")
        amount = request.get("amount", 7500)
        currency = request.get("currency", "usd")
        
        if not payment_token:
            raise HTTPException(status_code=400, detail="Payment token is required")
        
        result = await google_wallet_service.process_google_pay_payment(
            payment_token, amount, currency
        )
...
@api_router.post("/payments/google-pay/confirm")
async def confirm_google_pay_payment(request: dict):
    ...
    try:
        ...
        confirmation = await google_wallet_service.confirm_google_pay_payment(payment_intent_id)
        
        if confirmation.get("paid"):
            # Update session as paid
            if session_id:
                await db.sessions.update_one(
                    {"id": session_id},
                    {"$set": {
                        ...
                        "payment_confirmed_at": datetime.now().isoformat(),
                        "google_pay_payment_id": payment_intent_id
                    }}
                )
            
            return {"message": "Google Pay payment confirmed successfully", "paid": True}
        else:
            return {"message": "Google Pay payment confirmation failed", "paid": False}
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@api_router.get("/payments/google-pay/config")
async def get_google_pay_config():
    """Get Google Pay configuration for frontend"""
    try:
        config = google_wallet_service.get_google_pay_config()
        return config
